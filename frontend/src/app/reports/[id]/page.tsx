...
import { Send, ArrowLeft, Eye, Check, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div>
        <div className="flex items-start justify-between mb-6">
            <div>
                <Link href="/reports" className="flex items-center mb-2 text-sm text-gray-500 hover:text-gray-700"><ArrowLeft className="w-4 h-4 mr-1"/>Back to My Reports</Link>
                <h1 className="text-2xl font-semibold text-gray-900">{report.title}</h1>
            </div>
            {report.status === 'DRAFT' && (<button onClick={() => setSubmitModalOpen(true)} className="inline-flex items-center px-4 py-2 text-sm font-medium text-white rounded-md bg-primary hover:bg-primary-hover"><Send className="w-5 h-5 mr-2" />Submit for Approval</button>)}
        </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {report.expenses.map(exp => (
                        <tr key={exp.id}>
                            ...
                            <td className="px-6 py-4 text-sm text-right whitespace-nowrap">{exp.receipt_blob_id && (<button onClick={() => handleViewReceipt(exp.receipt_blob_id!)} className="text-primary hover:text-primary-hover" title="View Receipt"><Eye className="w-5 h-5" /></button>)}</td>
                        </tr>
                    ))}
                </tbody>
        ...
        {report.rejection_reason && (
            <div className="p-4 mt-6 text-red-700 bg-red-100 border-l-4 border-red-500" role="alert">
                <div className="flex"><div className="py-1"><AlertTriangle className="w-6 h-6 mr-4 text-red-500"/></div><div><p className="font-bold">This report was rejected</p><p className="text-sm">{report.rejection_reason}</p></div></div>
            </div>
        )}
